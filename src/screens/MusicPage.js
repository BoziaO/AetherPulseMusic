...
import { Play, Plus, Music as MusicIcon, List, TrendingUp, Heart, Clock } from "../components/Icons";
import PageSkeleton from "../components/PageSkeleton";
import AddTrackModal from "../components/AddTrackModal";
import { fetchJson } from "../lib/api";

export default function MusicPage({ pageKey }) {
  const { pageData, pageLoading, play, favoriteItems, recentPlays, toggleFavoriteTrack } = useOutletContext();
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="relative mb-12">
            <div className="flex flex-col md:flex-row items-end gap-8">
              <div className="w-48 h-48 md:w-64 md:h-64 rounded-[var(--radius-main)] overflow-hidden shadow-2xl flex-shrink-0 bg-gradient-to-br from-primary to-accent flex items-center justify-center">
                <Heart size={64} className="text-white opacity-60" fill="white" />
              </div>
              <div className="flex-1 min-w-0">
                <span className="text-xs font-black uppercase tracking-[0.3em] text-primary mb-3 block">Biblioteka</span>
                ...
                <p className="text-lg font-medium opacity-60 max-w-2xl mb-8 leading-relaxed">
                  Twoje ulubione utwory zapisane podczas słuchania.
                </p>
                <div className="flex flex-wrap items-center gap-4">
                  <button
                      onClick={() => favTracks.length > 0 && play(favTracks[0], favTracks)}
                      disabled={favTracks.length === 0}
                      className="px-8 py-4 bg-primary text-white rounded-full font-black uppercase tracking-widest flex items-center gap-3 hover:scale-110 active:scale-95 transition-all shadow-xl shadow-primary/20 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
                  >
                    <Play size={20} fill="white" />
                    Odtwórz wszystko
                  </button>
                  <div className="px-6 py-4 rounded-[var(--radius-main)] bg-white/5 border border-white/10">
                    <span className="block text-[10px] font-black uppercase tracking-widest opacity-40 mb-1">Ulubione</span>
                    <span className="text-lg font-bold">{favTracks.length}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {favTracks.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-32 gap-6 opacity-40">
                <Heart size={64} />
                <p className="text-xl font-bold">Brak ulubionych</p>
                <p className="text-sm opacity-70">Kliknij ❤ podczas odtwarzania, aby dodać utwór.</p>
              </div>
          ) : (
                ...
                {favTracks.map((track, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
                        <button
                            onClick={(e) => { e.stopPropagation(); toggleFavoriteTrack?.(track); }}
                            className="p-2 text-primary hover:scale-125 transition-all"
                            title="Usuń z ulubionych"
                        >
                          <Heart size={18} fill="currentColor" />
                        </button>
                        <button
                            onClick={(e) => { e.stopPropagation(); setSelectedTrackForAdd(track); setIsAddModalOpen(true); }}
                            className="p-2 hover:text-primary transition-colors"
                        >
                          <Plus size={18} />
                        </button>
                        <span className="text-xs font-black opacity-40 tabular-nums">{track.duration || ""}</span>
                      </div>
                    </div>
                ))}
              </div>
          ...
          {isAddModalOpen && (
              <AddTrackModal
                  onClose={() => { setIsAddModalOpen(false); setSelectedTrackForAdd(null); }}
  ...
  if (pageKey === "recent") {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="relative mb-12">
            <div className="flex flex-col md:flex-row items-end gap-8">
              <div className="w-48 h-48 md:w-64 md:h-64 rounded-[var(--radius-main)] overflow-hidden shadow-2xl flex-shrink-0 bg-gradient-to-br from-primary/60 to-accent flex items-center justify-center">
                <Clock size={64} className="text-white opacity-60" />
              </div>
              <div className="flex-1 min-w-0">
                <span className="text-xs font-black uppercase tracking-[0.3em] text-primary mb-3 block">Historia</span>
                <h1 className="text-5xl md:text-7xl font-black mb-4 tracking-tighter leading-none" style={{ color: "var(--text-main)" }}>
                  Ostatnio grane
                </h1>
                <p className="text-lg font-medium opacity-60 max-w-2xl mb-8 leading-relaxed">
                  Wróć do ostatnio odtwarzanych utworów. Historia przechowywana lokalnie.
                </p>
                <div className="flex flex-wrap items-center gap-4">
                  <button
                      onClick={() => recent.length > 0 && play(recent[0], recent)}
                  ...
                  <div className="px-6 py-4 rounded-[var(--radius-main)] bg-white/5 border border-white/10">
                    <span className="block text-[10px] font-black uppercase tracking-widest opacity-40 mb-1">Historia</span>
                    <span className="text-lg font-bold">{recent.length} / 25</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {recent.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-32 gap-6 opacity-40">
                <Clock size={64} />
                <p className="text-xl font-bold">Brak historii</p>
                <p className="text-sm opacity-70">Zacznij słuchać, aby historia się wypełniła.</p>
              </div>
          ) : (
              <div className="space-y-1">
                {recent.map((track, i) => (
  ...
  // ── All other pages (home, discover, playlists, albums, artists, artist/:id, album/:id) ──
                        ...
                        <div
                            ...
                            onClick={() => {
                              if (item.videoId) play(item);
                                ...
                                // Do not encodeURIComponent — React Router handles it
