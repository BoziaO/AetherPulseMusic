  ...
  return (
    <aside className="sidebar sidebar--no-scrollbar w-[280px] h-screen fixed left-0 top-0 overflow-y-auto bg-black border-r border-white/5 flex flex-col p-8 z-[110]">
      <div className="flex items-center gap-4 mb-12 group cursor-pointer">
        ...
        <div>
          <h2 className="text-2xl font-black text-white tracking-tighter uppercase italic">Bozia<span className="text-red-500">Music</span></h2>
      ...
      <nav className="flex-1 space-y-10">
        {navigationGroups.map((group) => (
          <div key={group.title} className="space-y-4">
            <p className="px-4 text-[10px] uppercase tracking-[0.25em] text-neutral-700 font-black">{group.title}</p>
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      <Icon size={20} className="group-hover:text-red-500 transition-colors duration-300" />
                      <div className="absolute inset-0 bg-red-500 blur-lg opacity-0 group-hover:opacity-20 transition-opacity"></div>
                    </div>
                    <span className="font-bold text-sm tracking-tight">{item.label}</span>
                    <div className="ml-auto w-1.5 h-1.5 rounded-full bg-red-500 scale-0 group-[.active]:scale-100 transition-transform duration-500"></div>
                  </NavLink>
                );
              })}
            </div>
      ...
      <div className="mt-12 pt-8 border-t border-white/5">
        <div className="bg-gradient-to-br from-neutral-900/50 to-black p-6 rounded-[32px] border border-white/5 relative overflow-hidden group">
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="px-2 py-0.5 bg-red-500/10 text-red-500 text-[9px] font-black uppercase tracking-widest rounded-full border border-red-500/20">System</span>
