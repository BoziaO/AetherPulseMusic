/**
 * Theme catalogue.
 *
 * Every theme supplies:
 *  - `id`          stable identifier (used in localStorage, URLs)
 *  - `name`        translation key in src/data/i18n.js (theme_<id>)
 *  - `mode`        "dark" | "light" — drives `color-scheme`
 *  - `vibe`        2–3 word descriptor shown in the picker
 *  - `swatch`      three hex colors used to render the preview swatch
 *  - `vars`        CSS custom properties applied to <html>
 *  - `effects?`    optional list of CSS classes added to <html>.dataset.themeFx
 *                  for theme-specific page animations (glitch, scanline, etc.)
 *
 * Adding a theme = append an entry here. The Settings page and theme engine
 * are fully data-driven.
 *
 * Typography vars (all themes carry these):
 *  --font-weight-display   weight for headings & track titles
 *  --font-weight-body      weight for body / artist names
 *  --icon-style            "outline" (all themes use Lucide outline)
 *  --icon-stroke-width     stroke-width passed to Lucide icons
 *  --letter-spacing        letter-spacing for the overall UI
 */

export const THEMES = [
  /* ------------------------------------------------------------------ */
  /*  Default Apple-Music-inspired palettes                             */
  /* ------------------------------------------------------------------ */
  {
    id: "apple-dark",
    name: "themeAppleDark",
    mode: "dark",
    vibe: "Clean · Dark",
    swatch: ["#000000", "#1c1c1e", "#fa243c"],
    vars: {
      "--bg-base": "#000000",
      "--bg-elevated": "#1c1c1e",
      "--bg-card": "rgba(28, 28, 30, 0.6)",
      "--bg-card-strong": "#1c1c1e",
      "--bg-hover": "rgba(255, 255, 255, 0.06)",
      "--bg-active": "rgba(255, 255, 255, 0.10)",
      "--bg-input": "rgba(255, 255, 255, 0.07)",
      "--bg-sidebar": "rgba(22, 22, 23, 0.85)",
      "--bg-player": "rgba(28, 28, 30, 0.92)",
      "--text-primary": "#ffffff",
      "--text-secondary": "rgba(255, 255, 255, 0.72)",
      "--text-tertiary": "rgba(255, 255, 255, 0.45)",
      "--text-quaternary": "rgba(255, 255, 255, 0.28)",
      "--line": "rgba(255, 255, 255, 0.08)",
      "--line-strong": "rgba(255, 255, 255, 0.14)",
      "--primary": "#fa243c",
      "--primary-hover": "#ff375f",
      "--primary-rgb": "250, 36, 60",
      "--accent": "#0a84ff",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2",
      "--letter-spacing": "-0.01em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  AMOLED — pure black, OLED-friendly, max contrast, no shadows      */
  /* ------------------------------------------------------------------ */
  {
    id: "amoled",
    name: "themeAmoled",
    mode: "dark",
    vibe: "Pure Black · OLED",
    swatch: ["#000000", "#0a0a0a", "#ffffff"],
    vars: {
      "--bg-base": "#000000",
      "--bg-elevated": "#000000",
      "--bg-card": "rgba(10, 10, 10, 0.6)",
      "--bg-card-strong": "#0a0a0a",
      "--bg-hover": "rgba(255, 255, 255, 0.06)",
      "--bg-active": "rgba(255, 255, 255, 0.12)",
      "--bg-input": "rgba(255, 255, 255, 0.05)",
      "--bg-sidebar": "#000000",
      "--bg-player": "#000000",
      "--text-primary": "#ffffff",
      "--text-secondary": "rgba(255, 255, 255, 0.82)",
      "--text-tertiary": "rgba(255, 255, 255, 0.55)",
      "--text-quaternary": "rgba(255, 255, 255, 0.32)",
      "--line": "rgba(255, 255, 255, 0.08)",
      "--line-strong": "rgba(255, 255, 255, 0.18)",
      "--primary": "#ff375f",
      "--primary-hover": "#ff5277",
      "--primary-rgb": "255, 55, 95",
      "--accent": "#0a84ff",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2",
      "--letter-spacing": "-0.01em",
      // AMOLED-specific: usuwamy cienie, zwiększamy kontrast
      "--shadow-card": "none",
      "--shadow-strong": "0 0 0 1px rgba(255, 255, 255, 0.06)",
    },
    effects: ["theme-amoled"],
  },
  {
    id: "apple-light",
    name: "themeAppleLight",
    mode: "light",
    vibe: "Clean · Light",
    swatch: ["#ffffff", "#f5f5f7", "#fa243c"],
    vars: {
      "--bg-base": "#ffffff",
      "--bg-elevated": "#f5f5f7",
      "--bg-card": "rgba(245, 245, 247, 0.7)",
      "--bg-card-strong": "#f5f5f7",
      "--bg-hover": "rgba(0, 0, 0, 0.04)",
      "--bg-active": "rgba(0, 0, 0, 0.08)",
      "--bg-input": "rgba(0, 0, 0, 0.05)",
      "--bg-sidebar": "rgba(245, 245, 247, 0.85)",
      "--bg-player": "rgba(255, 255, 255, 0.92)",
      "--text-primary": "#1d1d1f",
      "--text-secondary": "rgba(29, 29, 31, 0.72)",
      "--text-tertiary": "rgba(29, 29, 31, 0.5)",
      "--text-quaternary": "rgba(29, 29, 31, 0.3)",
      "--line": "rgba(0, 0, 0, 0.08)",
      "--line-strong": "rgba(0, 0, 0, 0.14)",
      "--primary": "#fa243c",
      "--primary-hover": "#ff375f",
      "--primary-rgb": "250, 36, 60",
      "--accent": "#0a84ff",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2",
      "--letter-spacing": "-0.01em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Cyberpunk: Edgerunners — yellow/magenta/cyan on deep violet black */
  /* ------------------------------------------------------------------ */
  {
    id: "edgerunners",
    name: "themeEdgerunners",
    mode: "dark",
    vibe: "Neon · Glitch",
    swatch: ["#0a0514", "#fcee0a", "#ff2e63"],
    vars: {
      "--bg-base": "#06030f",
      "--bg-elevated": "#120826",
      "--bg-card": "rgba(24, 12, 46, 0.65)",
      "--bg-card-strong": "#1a0d35",
      "--bg-hover": "rgba(252, 238, 10, 0.08)",
      "--bg-active": "rgba(252, 238, 10, 0.14)",
      "--bg-input": "rgba(252, 238, 10, 0.06)",
      "--bg-sidebar": "rgba(10, 5, 24, 0.85)",
      "--bg-player": "rgba(14, 6, 30, 0.92)",
      "--text-primary": "#fcee0a",
      "--text-secondary": "rgba(252, 238, 10, 0.76)",
      "--text-tertiary": "rgba(234, 210, 255, 0.55)",
      "--text-quaternary": "rgba(234, 210, 255, 0.3)",
      "--line": "rgba(255, 46, 99, 0.18)",
      "--line-strong": "rgba(0, 229, 255, 0.35)",
      "--primary": "#ff2e63",
      "--primary-hover": "#ff5c8a",
      "--primary-rgb": "255, 46, 99",
      "--accent": "#00e5ff",
      "--font-weight-display": "800",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.5",
      "--letter-spacing": "-0.02em",
    },
    effects: ["theme-glitch", "theme-scanlines"],
  },
  /* ------------------------------------------------------------------ */
  /*  Goth: deep blood red over obsidian, crimson blooms, serif vibe    */
  /* ------------------------------------------------------------------ */
  {
    id: "goth",
    name: "themeGoth",
    mode: "dark",
    vibe: "Crimson · Victorian",
    swatch: ["#0b0608", "#1a0a0c", "#8c0f1f"],
    vars: {
      "--bg-base": "#0a0607",
      "--bg-elevated": "#160a0c",
      "--bg-card": "rgba(26, 10, 14, 0.65)",
      "--bg-card-strong": "#1a0d10",
      "--bg-hover": "rgba(140, 15, 31, 0.08)",
      "--bg-active": "rgba(140, 15, 31, 0.15)",
      "--bg-input": "rgba(255, 236, 230, 0.04)",
      "--bg-sidebar": "rgba(12, 6, 7, 0.9)",
      "--bg-player": "rgba(18, 8, 10, 0.94)",
      "--text-primary": "#f4e5e8",
      "--text-secondary": "rgba(244, 229, 232, 0.72)",
      "--text-tertiary": "rgba(244, 229, 232, 0.42)",
      "--text-quaternary": "rgba(244, 229, 232, 0.22)",
      "--line": "rgba(140, 15, 31, 0.22)",
      "--line-strong": "rgba(140, 15, 31, 0.42)",
      "--primary": "#8c0f1f",
      "--primary-hover": "#c41e3a",
      "--primary-rgb": "140, 15, 31",
      "--accent": "#6b2f3e",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "0.01em",
    },
    effects: ["theme-flicker"],
  },
  /* ------------------------------------------------------------------ */
  /*  Metal: blackened steel, chrome edges, sharp crimson               */
  /* ------------------------------------------------------------------ */
  {
    id: "metal",
    name: "themeMetal",
    mode: "dark",
    vibe: "Industrial · Chrome",
    swatch: ["#0d0d0d", "#2a2a2d", "#e60000"],
    vars: {
      "--bg-base": "#0d0d0d",
      "--bg-elevated": "#1a1a1c",
      "--bg-card": "rgba(32, 32, 36, 0.65)",
      "--bg-card-strong": "#26262a",
      "--bg-hover": "rgba(230, 230, 230, 0.06)",
      "--bg-active": "rgba(230, 230, 230, 0.12)",
      "--bg-input": "rgba(230, 230, 230, 0.06)",
      "--bg-sidebar": "rgba(16, 16, 18, 0.9)",
      "--bg-player": "rgba(20, 20, 24, 0.94)",
      "--text-primary": "#f5f5f7",
      "--text-secondary": "rgba(245, 245, 247, 0.74)",
      "--text-tertiary": "rgba(245, 245, 247, 0.44)",
      "--text-quaternary": "rgba(245, 245, 247, 0.24)",
      "--line": "rgba(230, 0, 0, 0.16)",
      "--line-strong": "rgba(230, 0, 0, 0.36)",
      "--primary": "#e60000",
      "--primary-hover": "#ff1a1a",
      "--primary-rgb": "230, 0, 0",
      "--accent": "#cfcfd4",
      "--font-weight-display": "900",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2.5",
      "--letter-spacing": "0.02em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Kai Angel: blushed-pink, icy blue, dreamy haze                    */
  /* ------------------------------------------------------------------ */
  {
    id: "kai-angel",
    name: "themeKaiAngel",
    mode: "dark",
    vibe: "Dreampop · Pastel",
    swatch: ["#1a0820", "#ff9fd6", "#c7b0ff"],
    vars: {
      "--bg-base": "#14081b",
      "--bg-elevated": "#24112f",
      "--bg-card": "rgba(42, 20, 56, 0.6)",
      "--bg-card-strong": "#2d1640",
      "--bg-hover": "rgba(255, 159, 214, 0.08)",
      "--bg-active": "rgba(255, 159, 214, 0.16)",
      "--bg-input": "rgba(255, 214, 235, 0.06)",
      "--bg-sidebar": "rgba(18, 8, 24, 0.86)",
      "--bg-player": "rgba(24, 12, 32, 0.92)",
      "--text-primary": "#fdeaff",
      "--text-secondary": "rgba(253, 234, 255, 0.78)",
      "--text-tertiary": "rgba(253, 234, 255, 0.5)",
      "--text-quaternary": "rgba(253, 234, 255, 0.3)",
      "--line": "rgba(255, 159, 214, 0.2)",
      "--line-strong": "rgba(199, 176, 255, 0.4)",
      "--primary": "#ff9fd6",
      "--primary-hover": "#ffb3e0",
      "--primary-rgb": "255, 159, 214",
      "--accent": "#c7b0ff",
      "--font-weight-display": "600",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.5",
      "--letter-spacing": "-0.005em",
    },
    effects: ["theme-dream-haze"],
  },
  /* ------------------------------------------------------------------ */
  /*  Synthwave: sunset grid — magenta/purple/cyan                      */
  /* ------------------------------------------------------------------ */
  {
    id: "synthwave",
    name: "themeSynthwave",
    mode: "dark",
    vibe: "Retrowave · Sunset",
    swatch: ["#14052b", "#ff006e", "#00e5ff"],
    vars: {
      "--bg-base": "#0c0320",
      "--bg-elevated": "#1e0a3f",
      "--bg-card": "rgba(38, 13, 80, 0.65)",
      "--bg-card-strong": "#2a0e55",
      "--bg-hover": "rgba(255, 0, 110, 0.1)",
      "--bg-active": "rgba(255, 0, 110, 0.18)",
      "--bg-input": "rgba(255, 0, 110, 0.07)",
      "--bg-sidebar": "rgba(14, 5, 34, 0.86)",
      "--bg-player": "rgba(20, 8, 44, 0.92)",
      "--text-primary": "#fde8ff",
      "--text-secondary": "rgba(253, 232, 255, 0.78)",
      "--text-tertiary": "rgba(253, 232, 255, 0.5)",
      "--text-quaternary": "rgba(253, 232, 255, 0.28)",
      "--line": "rgba(255, 0, 110, 0.2)",
      "--line-strong": "rgba(0, 229, 255, 0.35)",
      "--primary": "#ff006e",
      "--primary-hover": "#ff3a8a",
      "--primary-rgb": "255, 0, 110",
      "--accent": "#00e5ff",
      "--font-weight-display": "800",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "-0.015em",
    },
    effects: ["theme-scanlines"],
  },
  /* ------------------------------------------------------------------ */
  /*  Vampire: deep burgundy, bone white, candle gold                   */
  /* ------------------------------------------------------------------ */
  {
    id: "vampire",
    name: "themeVampire",
    mode: "dark",
    vibe: "Bloodmoon · Baroque",
    swatch: ["#14070a", "#5a0d1b", "#f3e9d2"],
    vars: {
      "--bg-base": "#110608",
      "--bg-elevated": "#1f0a0e",
      "--bg-card": "rgba(36, 12, 18, 0.7)",
      "--bg-card-strong": "#2a0e14",
      "--bg-hover": "rgba(243, 233, 210, 0.06)",
      "--bg-active": "rgba(243, 233, 210, 0.12)",
      "--bg-input": "rgba(243, 233, 210, 0.05)",
      "--bg-sidebar": "rgba(17, 6, 8, 0.9)",
      "--bg-player": "rgba(22, 8, 12, 0.94)",
      "--text-primary": "#f3e9d2",
      "--text-secondary": "rgba(243, 233, 210, 0.76)",
      "--text-tertiary": "rgba(243, 233, 210, 0.48)",
      "--text-quaternary": "rgba(243, 233, 210, 0.28)",
      "--line": "rgba(90, 13, 27, 0.35)",
      "--line-strong": "rgba(90, 13, 27, 0.55)",
      "--primary": "#a51c2f",
      "--primary-hover": "#c72943",
      "--primary-rgb": "165, 28, 47",
      "--accent": "#c9a25a",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "0.005em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Matrix: classic terminal green on pitch black                     */
  /* ------------------------------------------------------------------ */
  {
    id: "matrix",
    name: "themeMatrix",
    mode: "dark",
    vibe: "Terminal · Lime",
    swatch: ["#000000", "#021a05", "#00ff66"],
    vars: {
      "--bg-base": "#000704",
      "--bg-elevated": "#03140a",
      "--bg-card": "rgba(5, 28, 12, 0.6)",
      "--bg-card-strong": "#071c0e",
      "--bg-hover": "rgba(0, 255, 102, 0.06)",
      "--bg-active": "rgba(0, 255, 102, 0.12)",
      "--bg-input": "rgba(0, 255, 102, 0.06)",
      "--bg-sidebar": "rgba(2, 10, 5, 0.9)",
      "--bg-player": "rgba(3, 20, 10, 0.94)",
      "--text-primary": "#a9ffc9",
      "--text-secondary": "rgba(169, 255, 201, 0.74)",
      "--text-tertiary": "rgba(169, 255, 201, 0.48)",
      "--text-quaternary": "rgba(169, 255, 201, 0.26)",
      "--line": "rgba(0, 255, 102, 0.16)",
      "--line-strong": "rgba(0, 255, 102, 0.36)",
      "--primary": "#00ff66",
      "--primary-hover": "#3bff88",
      "--primary-rgb": "0, 255, 102",
      "--accent": "#7dffb0",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2",
      "--letter-spacing": "0.02em",
    },
    effects: ["theme-scanlines"],
  },
  /* ------------------------------------------------------------------ */
  /*  Phonk: dark purple, neon orange, drift phonk vibes                */
  /* ------------------------------------------------------------------ */
  {
    id: "phonk",
    name: "themePhonk",
    mode: "dark",
    vibe: "Drift · Aggressive",
    swatch: ["#0a0510", "#ff6b1a", "#9d4edd"],
    vars: {
      "--bg-base": "#08030d",
      "--bg-elevated": "#140820",
      "--bg-card": "rgba(26, 12, 42, 0.65)",
      "--bg-card-strong": "#1e0e30",
      "--bg-hover": "rgba(255, 107, 26, 0.08)",
      "--bg-active": "rgba(255, 107, 26, 0.15)",
      "--bg-input": "rgba(255, 107, 26, 0.06)",
      "--bg-sidebar": "rgba(12, 5, 20, 0.9)",
      "--bg-player": "rgba(16, 8, 28, 0.94)",
      "--text-primary": "#ffeedd",
      "--text-secondary": "rgba(255, 238, 221, 0.76)",
      "--text-tertiary": "rgba(255, 238, 221, 0.48)",
      "--text-quaternary": "rgba(255, 238, 221, 0.28)",
      "--line": "rgba(255, 107, 26, 0.2)",
      "--line-strong": "rgba(255, 107, 26, 0.4)",
      "--primary": "#ff6b1a",
      "--primary-hover": "#ff8533",
      "--primary-rgb": "255, 107, 26",
      "--accent": "#9d4edd",
      "--font-weight-display": "900",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2.5",
      "--letter-spacing": "-0.02em",
    },
    effects: ["theme-glitch", "theme-bass-pulse"],
  },
  /* ------------------------------------------------------------------ */
  /*  Y2K: chrome, hot pink, futuristic millennium aesthetic            */
  /* ------------------------------------------------------------------ */
  {
    id: "y2k",
    name: "themeY2K",
    mode: "dark",
    vibe: "Futuristic · Chrome",
    swatch: ["#0a0a0f", "#ff1493", "#00ffff"],
    vars: {
      "--bg-base": "#08080d",
      "--bg-elevated": "#14141f",
      "--bg-card": "rgba(26, 26, 40, 0.65)",
      "--bg-card-strong": "#1e1e2e",
      "--bg-hover": "rgba(255, 20, 147, 0.08)",
      "--bg-active": "rgba(255, 20, 147, 0.15)",
      "--bg-input": "rgba(255, 20, 147, 0.06)",
      "--bg-sidebar": "rgba(12, 12, 18, 0.9)",
      "--bg-player": "rgba(16, 16, 24, 0.94)",
      "--text-primary": "#ffe6f5",
      "--text-secondary": "rgba(255, 230, 245, 0.78)",
      "--text-tertiary": "rgba(255, 230, 245, 0.5)",
      "--text-quaternary": "rgba(255, 230, 245, 0.3)",
      "--line": "rgba(255, 20, 147, 0.22)",
      "--line-strong": "rgba(0, 255, 255, 0.4)",
      "--primary": "#ff1493",
      "--primary-hover": "#ff33a8",
      "--primary-rgb": "255, 20, 147",
      "--accent": "#00ffff",
      "--font-weight-display": "800",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.5",
      "--letter-spacing": "-0.01em",
    },
    effects: ["theme-scanlines", "theme-chrome-glow"],
  },
  /* ------------------------------------------------------------------ */
  /*  Emo Revival: black, blood red, midwest emo nostalgia              */
  /* ------------------------------------------------------------------ */
  {
    id: "emo",
    name: "themeEmo",
    mode: "dark",
    vibe: "Raw · Emotional",
    swatch: ["#0a0a0a", "#8b0000", "#d4d4d4"],
    vars: {
      "--bg-base": "#080808",
      "--bg-elevated": "#141414",
      "--bg-card": "rgba(26, 26, 26, 0.65)",
      "--bg-card-strong": "#1a1a1a",
      "--bg-hover": "rgba(139, 0, 0, 0.08)",
      "--bg-active": "rgba(139, 0, 0, 0.15)",
      "--bg-input": "rgba(212, 212, 212, 0.06)",
      "--bg-sidebar": "rgba(10, 10, 10, 0.92)",
      "--bg-player": "rgba(14, 14, 14, 0.95)",
      "--text-primary": "#e8e8e8",
      "--text-secondary": "rgba(232, 232, 232, 0.74)",
      "--text-tertiary": "rgba(232, 232, 232, 0.46)",
      "--text-quaternary": "rgba(232, 232, 232, 0.26)",
      "--line": "rgba(139, 0, 0, 0.24)",
      "--line-strong": "rgba(139, 0, 0, 0.44)",
      "--primary": "#dc143c",
      "--primary-hover": "#ff1a4a",
      "--primary-rgb": "220, 20, 60",
      "--accent": "#8b0000",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2",
      "--letter-spacing": "0em",
    },
    effects: ["theme-flicker"],
  },
  /* ------------------------------------------------------------------ */
  /*  Industrial: steel gray, hazard yellow, mechanized                 */
  /* ------------------------------------------------------------------ */
  {
    id: "industrial",
    name: "themeIndustrial",
    mode: "dark",
    vibe: "Mechanical · Raw",
    swatch: ["#1a1a1a", "#ffd700", "#708090"],
    vars: {
      "--bg-base": "#141414",
      "--bg-elevated": "#1e1e1e",
      "--bg-card": "rgba(36, 36, 36, 0.7)",
      "--bg-card-strong": "#242424",
      "--bg-hover": "rgba(255, 215, 0, 0.06)",
      "--bg-active": "rgba(255, 215, 0, 0.12)",
      "--bg-input": "rgba(255, 215, 0, 0.05)",
      "--bg-sidebar": "rgba(18, 18, 18, 0.92)",
      "--bg-player": "rgba(22, 22, 22, 0.95)",
      "--text-primary": "#d8d8d8",
      "--text-secondary": "rgba(216, 216, 216, 0.76)",
      "--text-tertiary": "rgba(216, 216, 216, 0.48)",
      "--text-quaternary": "rgba(216, 216, 216, 0.28)",
      "--line": "rgba(255, 215, 0, 0.18)",
      "--line-strong": "rgba(255, 215, 0, 0.38)",
      "--primary": "#ffd700",
      "--primary-hover": "#ffe533",
      "--primary-rgb": "255, 215, 0",
      "--accent": "#708090",
      "--font-weight-display": "800",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2.5",
      "--letter-spacing": "0.04em",
    },
    effects: ["theme-scanlines"],
  },
  /* ------------------------------------------------------------------ */
  /*  Midnight Ocean: deep navy blues with glowing cyan                 */
  /* ------------------------------------------------------------------ */
  {
    id: "midnight-ocean",
    name: "themeMidnightOcean",
    mode: "dark",
    vibe: "Deep · Oceanic",
    swatch: ["#050d1a", "#0a1f3d", "#00d4ff"],
    vars: {
      "--bg-base": "#040c18",
      "--bg-elevated": "#081628",
      "--bg-card": "rgba(10, 24, 48, 0.65)",
      "--bg-card-strong": "#0c1e38",
      "--bg-hover": "rgba(0, 212, 255, 0.07)",
      "--bg-active": "rgba(0, 212, 255, 0.14)",
      "--bg-input": "rgba(0, 212, 255, 0.05)",
      "--bg-sidebar": "rgba(5, 12, 24, 0.88)",
      "--bg-player": "rgba(6, 14, 28, 0.94)",
      "--text-primary": "#e0f4ff",
      "--text-secondary": "rgba(224, 244, 255, 0.74)",
      "--text-tertiary": "rgba(224, 244, 255, 0.46)",
      "--text-quaternary": "rgba(224, 244, 255, 0.26)",
      "--line": "rgba(0, 212, 255, 0.14)",
      "--line-strong": "rgba(0, 212, 255, 0.3)",
      "--primary": "#00d4ff",
      "--primary-hover": "#33ddff",
      "--primary-rgb": "0, 212, 255",
      "--accent": "#0080ff",
      "--font-weight-display": "600",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.5",
      "--letter-spacing": "-0.01em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Forest: deep emerald & mossy greens with warm amber               */
  /* ------------------------------------------------------------------ */
  {
    id: "forest",
    name: "themeForest",
    mode: "dark",
    vibe: "Natural · Earthy",
    swatch: ["#0a1209", "#1a2e14", "#52c41a"],
    vars: {
      "--bg-base": "#080f07",
      "--bg-elevated": "#111e0f",
      "--bg-card": "rgba(20, 36, 16, 0.65)",
      "--bg-card-strong": "#182b13",
      "--bg-hover": "rgba(82, 196, 26, 0.07)",
      "--bg-active": "rgba(82, 196, 26, 0.14)",
      "--bg-input": "rgba(82, 196, 26, 0.05)",
      "--bg-sidebar": "rgba(10, 18, 8, 0.9)",
      "--bg-player": "rgba(12, 20, 10, 0.94)",
      "--text-primary": "#d6f0c8",
      "--text-secondary": "rgba(214, 240, 200, 0.76)",
      "--text-tertiary": "rgba(214, 240, 200, 0.48)",
      "--text-quaternary": "rgba(214, 240, 200, 0.28)",
      "--line": "rgba(82, 196, 26, 0.16)",
      "--line-strong": "rgba(82, 196, 26, 0.32)",
      "--primary": "#52c41a",
      "--primary-hover": "#73d13d",
      "--primary-rgb": "82, 196, 26",
      "--accent": "#fa8c16",
      "--font-weight-display": "600",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "-0.005em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Aurora: deep violet with shifting aurora greens & purples         */
  /* ------------------------------------------------------------------ */
  {
    id: "aurora",
    name: "themeAurora",
    mode: "dark",
    vibe: "Northern · Celestial",
    swatch: ["#080c18", "#00ff87", "#7b2fff"],
    vars: {
      "--bg-base": "#060a14",
      "--bg-elevated": "#0e1428",
      "--bg-card": "rgba(16, 20, 44, 0.65)",
      "--bg-card-strong": "#141a38",
      "--bg-hover": "rgba(0, 255, 135, 0.07)",
      "--bg-active": "rgba(0, 255, 135, 0.14)",
      "--bg-input": "rgba(0, 255, 135, 0.05)",
      "--bg-sidebar": "rgba(8, 12, 22, 0.9)",
      "--bg-player": "rgba(8, 12, 22, 0.95)",
      "--text-primary": "#e8fff5",
      "--text-secondary": "rgba(232, 255, 245, 0.76)",
      "--text-tertiary": "rgba(232, 255, 245, 0.48)",
      "--text-quaternary": "rgba(232, 255, 245, 0.28)",
      "--line": "rgba(0, 255, 135, 0.16)",
      "--line-strong": "rgba(123, 47, 255, 0.4)",
      "--primary": "#00ff87",
      "--primary-hover": "#33ff9f",
      "--primary-rgb": "0, 255, 135",
      "--accent": "#7b2fff",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.5",
      "--letter-spacing": "-0.015em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Lo-Fi Café: warm amber & brown, cozy analog warmth                */
  /* ------------------------------------------------------------------ */
  {
    id: "lofi",
    name: "themeLofi",
    mode: "dark",
    vibe: "Warm · Chill",
    swatch: ["#120c05", "#2a1a0a", "#e8a857"],
    vars: {
      "--bg-base": "#120c05",
      "--bg-elevated": "#1e1208",
      "--bg-card": "rgba(38, 22, 8, 0.65)",
      "--bg-card-strong": "#2a1a0a",
      "--bg-hover": "rgba(232, 168, 87, 0.07)",
      "--bg-active": "rgba(232, 168, 87, 0.14)",
      "--bg-input": "rgba(232, 168, 87, 0.06)",
      "--bg-sidebar": "rgba(16, 10, 4, 0.9)",
      "--bg-player": "rgba(20, 13, 5, 0.94)",
      "--text-primary": "#f0deb8",
      "--text-secondary": "rgba(240, 222, 184, 0.76)",
      "--text-tertiary": "rgba(240, 222, 184, 0.46)",
      "--text-quaternary": "rgba(240, 222, 184, 0.26)",
      "--line": "rgba(232, 168, 87, 0.16)",
      "--line-strong": "rgba(232, 168, 87, 0.32)",
      "--primary": "#e8a857",
      "--primary-hover": "#f0bc74",
      "--primary-rgb": "232, 168, 87",
      "--accent": "#7eb87e",
      "--font-weight-display": "600",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "-0.005em",
    },
    effects: ["theme-warm-glow"],
  },
  /* ------------------------------------------------------------------ */
  /*  Neon City: Hong Kong/Tokyo neon — hot pink & electric yellow       */
  /* ------------------------------------------------------------------ */
  {
    id: "neon-city",
    name: "themeNeonCity",
    mode: "dark",
    vibe: "Urban · Electric",
    swatch: ["#060210", "#ff0090", "#ffe600"],
    vars: {
      "--bg-base": "#060210",
      "--bg-elevated": "#0e0720",
      "--bg-card": "rgba(20, 10, 38, 0.65)",
      "--bg-card-strong": "#180c2e",
      "--bg-hover": "rgba(255, 0, 144, 0.08)",
      "--bg-active": "rgba(255, 0, 144, 0.16)",
      "--bg-input": "rgba(255, 0, 144, 0.06)",
      "--bg-sidebar": "rgba(8, 4, 16, 0.9)",
      "--bg-player": "rgba(10, 5, 20, 0.94)",
      "--text-primary": "#fff0f8",
      "--text-secondary": "rgba(255, 240, 248, 0.76)",
      "--text-tertiary": "rgba(255, 240, 248, 0.46)",
      "--text-quaternary": "rgba(255, 240, 248, 0.26)",
      "--line": "rgba(255, 0, 144, 0.2)",
      "--line-strong": "rgba(255, 230, 0, 0.35)",
      "--primary": "#ff0090",
      "--primary-hover": "#ff33a8",
      "--primary-rgb": "255, 0, 144",
      "--accent": "#ffe600",
      "--font-weight-display": "800",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2",
      "--letter-spacing": "-0.015em",
    },
    effects: ["theme-scanlines", "theme-chrome-glow"],
  },
  /* ------------------------------------------------------------------ */
  /*  Amber Dusk: warm sunset orange & golden hour glow                 */
  /* ------------------------------------------------------------------ */
  {
    id: "amber-dusk",
    name: "themeAmberDusk",
    mode: "dark",
    vibe: "Sunset · Golden",
    swatch: ["#0e0504", "#261008", "#ff6b35"],
    vars: {
      "--bg-base": "#0e0504",
      "--bg-elevated": "#1c0c08",
      "--bg-card": "rgba(34, 14, 10, 0.65)",
      "--bg-card-strong": "#261008",
      "--bg-hover": "rgba(255, 107, 53, 0.08)",
      "--bg-active": "rgba(255, 107, 53, 0.15)",
      "--bg-input": "rgba(255, 107, 53, 0.06)",
      "--bg-sidebar": "rgba(12, 5, 4, 0.9)",
      "--bg-player": "rgba(16, 7, 5, 0.94)",
      "--text-primary": "#fff4e8",
      "--text-secondary": "rgba(255, 244, 232, 0.76)",
      "--text-tertiary": "rgba(255, 244, 232, 0.46)",
      "--text-quaternary": "rgba(255, 244, 232, 0.26)",
      "--line": "rgba(255, 107, 53, 0.18)",
      "--line-strong": "rgba(255, 201, 71, 0.35)",
      "--primary": "#ff6b35",
      "--primary-hover": "#ff8752",
      "--primary-rgb": "255, 107, 53",
      "--accent": "#ffc947",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "2",
      "--letter-spacing": "-0.01em",
    },
    effects: ["theme-dream-haze"],
  },
  /* ------------------------------------------------------------------ */
  /*  Nord: arctic blue, slate, frost — popular editor palette          */
  /* ------------------------------------------------------------------ */
  {
    id: "nord",
    name: "themeNord",
    mode: "dark",
    vibe: "Arctic · Minimal",
    swatch: ["#2e3440", "#5e81ac", "#88c0d0"],
    vars: {
      "--bg-base": "#242933",
      "--bg-elevated": "#2e3440",
      "--bg-card": "rgba(46, 52, 64, 0.7)",
      "--bg-card-strong": "#353c4a",
      "--bg-hover": "rgba(136, 192, 208, 0.07)",
      "--bg-active": "rgba(136, 192, 208, 0.14)",
      "--bg-input": "rgba(136, 192, 208, 0.06)",
      "--bg-sidebar": "rgba(36, 41, 51, 0.9)",
      "--bg-player": "rgba(38, 44, 56, 0.94)",
      "--text-primary": "#eceff4",
      "--text-secondary": "rgba(236, 239, 244, 0.78)",
      "--text-tertiary": "rgba(236, 239, 244, 0.5)",
      "--text-quaternary": "rgba(236, 239, 244, 0.3)",
      "--line": "rgba(94, 129, 172, 0.18)",
      "--line-strong": "rgba(136, 192, 208, 0.3)",
      "--primary": "#88c0d0",
      "--primary-hover": "#9ecfdc",
      "--primary-rgb": "136, 192, 208",
      "--accent": "#5e81ac",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "-0.01em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Catppuccin Mocha: warm muted dark — the beloved pastel palette    */
  /* ------------------------------------------------------------------ */
  {
    id: "catppuccin",
    name: "themeCatppuccin",
    mode: "dark",
    vibe: "Pastel · Cozy",
    swatch: ["#1e1e2e", "#cba6f7", "#89b4fa"],
    vars: {
      "--bg-base": "#181825",
      "--bg-elevated": "#1e1e2e",
      "--bg-card": "rgba(36, 36, 54, 0.7)",
      "--bg-card-strong": "#262637",
      "--bg-hover": "rgba(203, 166, 247, 0.07)",
      "--bg-active": "rgba(203, 166, 247, 0.14)",
      "--bg-input": "rgba(203, 166, 247, 0.06)",
      "--bg-sidebar": "rgba(24, 24, 37, 0.92)",
      "--bg-player": "rgba(22, 22, 34, 0.95)",
      "--text-primary": "#cdd6f4",
      "--text-secondary": "rgba(205, 214, 244, 0.78)",
      "--text-tertiary": "rgba(205, 214, 244, 0.5)",
      "--text-quaternary": "rgba(205, 214, 244, 0.3)",
      "--line": "rgba(203, 166, 247, 0.14)",
      "--line-strong": "rgba(137, 180, 250, 0.28)",
      "--primary": "#cba6f7",
      "--primary-hover": "#d4b5f8",
      "--primary-rgb": "203, 166, 247",
      "--accent": "#89b4fa",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "-0.01em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Rosé Pine: muted rose & pine — elegant, warm dark                 */
  /* ------------------------------------------------------------------ */
  {
    id: "rose-pine",
    name: "themeRosePine",
    mode: "dark",
    vibe: "Muted · Elegant",
    swatch: ["#191724", "#eb6f92", "#ebbcba"],
    vars: {
      "--bg-base": "#13111e",
      "--bg-elevated": "#1f1d2e",
      "--bg-card": "rgba(38, 35, 58, 0.7)",
      "--bg-card-strong": "#26233a",
      "--bg-hover": "rgba(235, 111, 146, 0.07)",
      "--bg-active": "rgba(235, 111, 146, 0.14)",
      "--bg-input": "rgba(235, 111, 146, 0.06)",
      "--bg-sidebar": "rgba(19, 17, 30, 0.92)",
      "--bg-player": "rgba(16, 14, 26, 0.95)",
      "--text-primary": "#e0def4",
      "--text-secondary": "rgba(224, 222, 244, 0.78)",
      "--text-tertiary": "rgba(224, 222, 244, 0.5)",
      "--text-quaternary": "rgba(224, 222, 244, 0.3)",
      "--line": "rgba(235, 111, 146, 0.16)",
      "--line-strong": "rgba(235, 188, 186, 0.28)",
      "--primary": "#eb6f92",
      "--primary-hover": "#f08aaa",
      "--primary-rgb": "235, 111, 146",
      "--accent": "#ebbcba",
      "--font-weight-display": "600",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "-0.005em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Discord: blurple navy — familiar dark UI feel                     */
  /* ------------------------------------------------------------------ */
  {
    id: "discord",
    name: "themeDiscord",
    mode: "dark",
    vibe: "Social · Navy",
    swatch: ["#1e2124", "#5865f2", "#7289da"],
    vars: {
      "--bg-base": "#1e2124",
      "--bg-elevated": "#282b30",
      "--bg-card": "rgba(44, 47, 54, 0.7)",
      "--bg-card-strong": "#36393f",
      "--bg-hover": "rgba(88, 101, 242, 0.1)",
      "--bg-active": "rgba(88, 101, 242, 0.18)",
      "--bg-input": "rgba(88, 101, 242, 0.07)",
      "--bg-sidebar": "rgba(30, 33, 36, 0.94)",
      "--bg-player": "rgba(28, 31, 34, 0.96)",
      "--text-primary": "#dcddde",
      "--text-secondary": "rgba(220, 221, 222, 0.78)",
      "--text-tertiary": "rgba(220, 221, 222, 0.5)",
      "--text-quaternary": "rgba(220, 221, 222, 0.3)",
      "--line": "rgba(88, 101, 242, 0.16)",
      "--line-strong": "rgba(114, 137, 218, 0.3)",
      "--primary": "#5865f2",
      "--primary-hover": "#6e7bf4",
      "--primary-rgb": "88, 101, 242",
      "--accent": "#7289da",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "0em",
    },
  },
  /* ------------------------------------------------------------------ */
  /*  Rose Light: soft blush pink light mode, editorial feel            */
  /* ------------------------------------------------------------------ */
  {
    id: "rose-light",
    name: "themeRoseLight",
    mode: "light",
    vibe: "Blush · Editorial",
    swatch: ["#fdf6f8", "#f8e8ee", "#e8476a"],
    vars: {
      "--bg-base": "#fdf6f8",
      "--bg-elevated": "#f8eff3",
      "--bg-card": "rgba(248, 239, 243, 0.8)",
      "--bg-card-strong": "#f5e8ed",
      "--bg-hover": "rgba(232, 71, 106, 0.05)",
      "--bg-active": "rgba(232, 71, 106, 0.1)",
      "--bg-input": "rgba(232, 71, 106, 0.06)",
      "--bg-sidebar": "rgba(253, 246, 248, 0.9)",
      "--bg-player": "rgba(255, 252, 254, 0.96)",
      "--text-primary": "#2a1520",
      "--text-secondary": "rgba(42, 21, 32, 0.72)",
      "--text-tertiary": "rgba(42, 21, 32, 0.5)",
      "--text-quaternary": "rgba(42, 21, 32, 0.3)",
      "--line": "rgba(232, 71, 106, 0.1)",
      "--line-strong": "rgba(232, 71, 106, 0.2)",
      "--primary": "#e8476a",
      "--primary-hover": "#ee6282",
      "--primary-rgb": "232, 71, 106",
      "--accent": "#c4326e",
      "--font-weight-display": "700",
      "--font-weight-body": "400",
      "--icon-style": "outline",
      "--icon-stroke-width": "1.75",
      "--letter-spacing": "-0.01em",
    },
  },
];

/** Quick lookup by id. */
export const THEME_MAP = Object.fromEntries(THEMES.map((t) => [t.id, t]));

/** Default theme when none is stored. */
export const DEFAULT_THEME_ID = "apple-dark";

/** Legacy ids that used to live in localStorage under `ap:theme`. */
const LEGACY_MAP = {
  dark: "apple-dark",
  light: "apple-light",
};

export function resolveThemeId(stored) {
  if (!stored) return DEFAULT_THEME_ID;
  if (THEME_MAP[stored]) return stored;
  if (LEGACY_MAP[stored]) return LEGACY_MAP[stored];
  return DEFAULT_THEME_ID;
}

export function getTheme(id) {
  return THEME_MAP[resolveThemeId(id)];
}
